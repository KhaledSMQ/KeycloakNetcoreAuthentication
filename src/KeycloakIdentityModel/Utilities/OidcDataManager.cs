...
namespace KeycloakIdentityModel.Utilities
{
    public class OidcDataManager
    {
        ...
        {
            var context = GetCachedContext(options.AuthenticationScheme);
        ...
        {
            return GetCachedContext(options.AuthenticationScheme);
        }
        ...
        {
            var context = GetCachedContextSafe(options.AuthenticationScheme);
            ...
            bool preload = true)
        {
            ...
            OidcManagerCache[options.AuthenticationScheme + CachedContextPostfix] = newContext;
        ...
        {
            return new Uri(baseUri.GetComponents(UriComponents.Scheme | UriComponents.StrongAuthority, UriFormat.Unescaped) + _options.CallbackPath);
        }
        ...
        public HttpContent BuildROPCAccessTokenEndpointContent(string username, string password)
        {
            // Create parameter dictionary
            var parameters = new Dictionary<string, string>
            {
            ...
            string postLogoutRedirectUrl = null)
        {
            ...
            if (string.IsNullOrEmpty(postLogoutRedirectUrl)) // Double-check options for empty/null
                postLogoutRedirectUrl = requestUri.GetComponents(UriComponents.StrongAuthority, UriFormat.Unescaped);
            else if (Uri.IsWellFormedUriString(postLogoutRedirectUrl, UriKind.Relative))
                postLogoutRedirectUrl = requestUri.GetComponents(UriComponents.StrongAuthority, UriFormat.Unescaped) + postLogoutRedirectUrl;
